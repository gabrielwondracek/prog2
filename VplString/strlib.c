...
char *str_concatenate(const char *stra, const char *strb) {
    ...
    size_t len_a = str_length(stra);
    size_t len_b = str_length(strb);

    char *resultado = malloc(len_a + len_b + 1);
    if (resultado == NULL) return NULL;

    char *dest = resultado;
    for (const char *ptr = stra; *ptr != '\0'; ptr++, dest++) {
        *dest = *ptr;
    }

    for (const char *ptr = strb; *ptr != '\0'; ptr++, dest++) {
        *dest = *ptr;
    }
...
int str_find_last(const char *str, const char c) {
    ...
    return (c == '\0') ? str_length(str) : last_pos;
}
